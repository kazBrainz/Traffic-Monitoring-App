...
document.getElementById('mode-select').addEventListener('change', calculateRoute);
  ...
  let updateContent = `The area around ${origin} is currently ${randomTraffic}. There are ${randomRoad} in the vicinity. Safety update: ${randomSafety}.`;
  ...
  if (destination) {
    ...
    updateContent += ` The route from ${origin} to ${destination} ${modeText[mode]} is ${distance} long and will take approximately ${duration}.`;
  }
